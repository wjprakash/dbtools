...
public class DerbyNetworkSqlHelper implements ISqlHelper {

	public String generateSelectStatement(ITable table, int currentRow,
			int pageSize) {
		StringBuffer selectBuffer = new StringBuffer();
		selectBuffer
				.append("SELECT * FROM (SELECT ROW_NUMBER() OVER () AS rownum, "); //$NON-NLS-1$
		selectBuffer.append(table.getQualifiedName() + ".* "); //$NON-NLS-1$
		selectBuffer.append(" FROM " + table.getQualifiedName()); //$NON-NLS-1$
		selectBuffer.append(") AS tmp WHERE rownum > " + currentRow); //$NON-NLS-1$
		selectBuffer.append(" AND rownum <=  " + (currentRow + pageSize)); //$NON-NLS-1$
		// ErrorManager.showInformationMessage(selectBuffer.toString());
			...
			for (IColumn column : columnList) {
				...
				if (isAutoIncrementSupported(column)) {
					if (column.isAutoIncrement()) {
						tableCreateBuffer
								.append(" GENERATED ALWAYS AS IDENTITY "); //$NON-NLS-1$
	...
	public String generateTableRowInsertStatement(ITableRowData tableRowData) {
		...
		for (int i = 0; i < tableRowData.getColumnCount(); i++) {
			...
			if (!columnData.isAutoIncrement() && !columnData.isReadOnly()
					...
					&& !columnData.getValueAsString().trim().equals("")) { //$NON-NLS-1$
	...
	public String generateCreateDatabaseStatement(String newDbName,
		...
		StringBuffer sb = new StringBuffer();
		sb.append("CREATE SCHEMA "); //$NON-NLS-1$
		sb.append(newDbName);
		return sb.toString();
	}
	...
	public String generateDropDatabaseStatement(ISchema schema) {
		...
		StringBuffer sb = new StringBuffer();
		try {
			for (ITable table : schema.getTableList()) {
				sb.append(generateTableDropStatement(table) + "\n"); //$NON-NLS-1$
			}
			sb.append("DROP SCHEMA "); //$NON-NLS-1$
			sb.append(schema.getName());
			sb.append(" RESTRICT"); //$NON-NLS-1$
			return sb.toString();
	...
	private String getQuotedValue(ITableColumnData tableColumnData) {
		if ((tableColumnData.getValueAsString() != null)
				&& !tableColumnData.getValueAsString().trim().equals("")) { //$NON-NLS-1$
			String prefixQuoteString = null;
			String suffixQuoteString = null;

			if (tableColumnData.getColumn() != null) {
				ISqlType sqltype = tableColumnData.getColumn().getType();
				prefixQuoteString = sqltype.getLiteralPrefix();
				suffixQuoteString = sqltype.getLiteralSuffix();
			}

			// Strange getLiteralPrefix() returns appended
			// with characters causes SQL execution to fail. Strip it
			if ((prefixQuoteString != null)
					&& prefixQuoteString.contains("X")) { //$NON-NLS-1$
				prefixQuoteString = prefixQuoteString.replaceAll("X", ""); //$NON-NLS-1$ //$NON-NLS-2$
			}
			if ((prefixQuoteString != null)
					&& prefixQuoteString.contains("DATE")) { //$NON-NLS-1$
				prefixQuoteString = prefixQuoteString.replaceAll("DATE", ""); //$NON-NLS-1$ //$NON-NLS-2$
			}
			if ((prefixQuoteString != null)
					&& prefixQuoteString.contains("TIMESTAMP")) { //$NON-NLS-1$
				prefixQuoteString = prefixQuoteString.replaceAll(
						"TIMESTAMP", ""); //$NON-NLS-1$ //$NON-NLS-2$
			}
			if ((prefixQuoteString != null)
					&& prefixQuoteString.contains("TIME")) { //$NON-NLS-1$
				prefixQuoteString = prefixQuoteString.replaceAll("TIME", ""); //$NON-NLS-1$ //$NON-NLS-2$
			}
